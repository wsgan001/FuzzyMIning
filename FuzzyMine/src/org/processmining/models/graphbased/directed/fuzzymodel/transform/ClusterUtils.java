...
public class ClusterUtils {
	...
	public static double calculateDistance(FMNode a, FMNode b) {
	...
	public static double calculateDistance(FMClusterNode cluster, FMNode node) {
		...
		double distance;
		for (FMNode other : cluster.getPrimitives()) {
	...
	public static double calculateDistance(FMClusterNode a, FMClusterNode b) {
		double minDistance = 1.0;
		double distance;
		for (FMNode node : a.getPrimitives()) {
	...
	public static double calculateDiameter(FMClusterNode cluster) {
		double diameter = 1.0;
		FMNode[] clusterNodeSet = cluster.getPrimitives().toArray(new FMNode[0]);
		ArrayList<FMNode> path;
		double dist;
	...
	protected static ArrayList<FMNode> findPath(FMNode a, FMNode b, FMNode[] nodeSet, int maxDepth) {
		FuzzyGraph graph = a.getGraph();
		ArrayList<FMNode> path = new ArrayList<FMNode>();
		int indexA = a.getIndex();
		int indexB = b.getIndex();
		if (graph.getBinarySignificance(indexA, indexB) > 0.0) {
			...
			ArrayList<FMNode> bestPath = null;
			ArrayList<FMNode> currentPath = null;
	...
	protected static double getPathCorrelation(ArrayList<FMNode> path) {
		FuzzyGraph graph = path.get(0).getGraph();
		double minCor = 1.0;
		double curCor;
		FMNode pre = path.get(0);
		FMNode post;
