...
import org.processmining.models.graphbased.directed.fuzzymodel.metrics.binary.DataTypeCorrelationMetric;
import org.processmining.models.graphbased.directed.fuzzymodel.metrics.binary.DataValueCorrelationMetric;
import org.processmining.models.graphbased.directed.fuzzymodel.metrics.binary.DistanceSignificanceMetric;
import org.processmining.models.graphbased.directed.fuzzymodel.metrics.binary.EndpointCorrelationMetric;
import org.processmining.models.graphbased.directed.fuzzymodel.metrics.binary.FrequencySignificanceMetric;
import org.processmining.models.graphbased.directed.fuzzymodel.metrics.binary.OriginatorCorrelationMetric;
import org.processmining.models.graphbased.directed.fuzzymodel.metrics.binary.ProximityCorrelationMetric;
...
import org.processmining.models.graphbased.directed.fuzzymodel.metrics.unary.AggregateUnaryMetric;
import org.processmining.models.graphbased.directed.fuzzymodel.metrics.unary.FrequencySignificanceUnaryMetric;
import org.processmining.models.graphbased.directed.fuzzymodel.metrics.unary.RoutingSignificanceMetric;
...
public class MetricsRepository {
	...
	public static MetricsRepository createRepository(XLogInfo logSummary) {
		...
		repository.addMetric(new ProximityCorrelationMetric(numberOfEvents));//临近点
		repository.addMetric(new EndpointCorrelationMetric(numberOfEvents));//终结点相关性
		repository.addMetric(new OriginatorCorrelationMetric(numberOfEvents));//起源点相关性
		repository.addMetric(new DataTypeCorrelationMetric(numberOfEvents));//数据类型相关性
		repository.addMetric(new DataValueCorrelationMetric(numberOfEvents));//数据值相关性
	 ...
	 */
		...
		public void apply(XLog log, Attenuation att, int maxRelationDistance) 
				throws IndexOutOfBoundsException {
			//get LogInfo
			//XLogInfo logSummary = XLogInfoFactory.createLogInfo(log);
			//int numOfEventClasses = logSummary.getEventClasses().size();
			...
			this.log = log;
			// run trace metrics
			System.out.println("number of traceMetrics:"+traceMetrics.size() );
			System.out.println("number of unaryDerivateMetrics:"+unaryDerivateMetrics.size() );
			System.out.println("number of binaryDerivateMetrics:"+binaryDerivateMetrics.size() );
			for(TraceMetric metric : traceMetrics) {
				//System.out.println("=====");
				metric.measure(log);
				//currentProgress++;
			...
			// derive primary log metrics using scanner
			LogScanner scanner = new LogScanner();
			scanner.scan(log, this, att, maxRelationDistance, null, 0);
			//currentProgress += numOfEventClasses;
